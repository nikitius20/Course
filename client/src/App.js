...
import {getTotalScore,getTotalQuest,getTotalPoints,getTotalCorrectPoints} from "./getTotal";
...
function App() {
  ...
  const GetListOfQuestions = ()=>{
    Axios.get("http://localhost:3001/getAllQuestions")
    .then((response)=>{
  ...
  const GetCompleteTests = ()=>{
    Axios.get("http://localhost:3001/getTests",{params:{userId:user.userId}})
    .then((response)=>{
    ...
    data-testid="app"> 
      ...
      </div>
      {
        ...
        className="menu-buttons">
          ...
          }> Get All Questions</button>
          <button 
            className="menu-button"
            data-testid="button"
            onClick={()=>setAction("GetTest")}> Get Test</button>
          <button 
            className="menu-button"
            data-testid="button"
            onClick={()=>{
        ...
        <div className="AllTestResults">
          {completeTests.map((answer,index)=>{
            return <div>
              <div>Test №{index}, theme ={answer.theme}
              </div>
              <ol>
